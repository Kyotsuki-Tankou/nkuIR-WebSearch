...
def conduct_basic_query(query_word_term=[],query_word_phrase=[],
                        es=es,index_name=index_name,fields=['title','anchor','content','url'],
                        query_size=5):
...
def do_search():
    data=request.json
    exact_search=data.get('exactSearh','')
    fuzzy_search=data.get('fuzzySearch','')
    
    query_word_term=list(set(exact_search.split('^')))
    query_word_term=[s for s in query_word_term if s.strip()]
    
    query_word_phrase=list(set(fuzzy_search.split('^')))
    ...
    query_cnt,query_list,result_list=conduct_basic_query(query_word_term=query_word_term,query_word_phrase=query_word_phrase,
    query_size=10)
...
if __name__=="__main__":
    while True:
        ...
        #     query_num,query_list,result_list=conduct_basic_query(query_word_term=query_word_term,query_word_phrase=query_word_phrase,
        #                               query_size=5)
        # else:
        #     query_num,query_list,result_list=conduct_regex_query(query_word_term=query_word_term,query_word_phrase=query_word_phrase,
        #                               query_word_regex=query_word_regex,query_size=5)  
        if len(query_word_regex)==0:
            query_num,query_list,result_list=conduct_basic_query(query_word_term=query_word_term,query_word_phrase=query_word_phrase,
                                      query_size=10)
        else:
            query_num,query_list=conduct_regex_query(query_word_term=query_word_term,query_word_phrase=query_word_phrase,
                                      query_word_regex=query_word_regex,query_size=10)  
        query_list=dict(sorted(query_list.items(),key=lambda item:item[1], reverse=True))
